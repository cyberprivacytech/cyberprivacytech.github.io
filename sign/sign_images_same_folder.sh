...
for IMAGE in *.{jpg,jpeg,png,gif,bmp,tiff}; do
    if [ -f "$IMAGE" ]; then
        ...
        gpg --yes --clearsign --default-key "$GPG_KEY" --output "$OUTPUT_SIGNED_METADATA" "$METADATA_FILE"
        ...
        exiftool -Comment="$SIGNED_CONTENT" -UserComment="$SIGNED_CONTENT" -XPComment="$SIGNED_CONTENT" -Description="$SIGNED_CONTENT" "$IMAGE"
